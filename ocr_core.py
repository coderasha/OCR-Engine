...
async def upload_file(file: UploadFile = File(...)):
    ...
    try:
        if file.filename.endswith(".pdf"):
            return JSONResponse(content={"error": "PDF not supported yet in AI mode. Please upload image."}, status_code=400)

        result_text = run_donut_ocr(file_path)

        # Simple pattern extraction from AI result
        import re
        fields = {}
